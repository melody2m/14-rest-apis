...
app.get('/api/v1/books/find', (req, res) => {
  ...
  // It populates the query with information given from the request. It adds properties to the query based on the request. 
  let query = ''
  ...
  // Superagent is a proxy that is used to authenticate the query before data is returned. 
  superagent.get(url)
    .query({'q': query})
    .query({'key': API_KEY})
    .then(response => response.body.items.map((book, idx) => {
      ...
      // Destructuring unpacks properties or identifiers from a larger object or collection. In this case, the properties of book.volumeInfo are being destructured into separate items. 
      let { title, authors, industryIdentifiers, imageLinks, description } = book.volumeInfo;

      // COMMENT: What is the purpose of the following placeholder image?
      // If no image is available from the API, this image will show.
      let placeholderImage = 'http://www.newyorkpaddy.com/images/covers/NoCoverAvailable.jpg';

      // COMMENT: Explain how ternary operators are being used below.
      // Ternary operators are being used to show book information if it is available, or if it is not, to dispaly instead a message that explains this to the user, fillin in the string.
      return {
...
// This sets up another route that depends on the isbn that is sourced from the client's request. It passes this information into the API request and only returns the book that corresponds to the ISBN. Instead of getting information from object sent in the request, it gets the context parameters. 
